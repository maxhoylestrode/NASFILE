...
function ResumePicker({ record, onDone }: { record: UploadRecord; onDone: () => void }) {
  ...
  return (
    <div>
      ...
      />
      <button onClick={() => inputRef.current?.click()} className="text-xs font-medium text-indigo-600 hover:underline">
    ...
    <div className="fixed bottom-4 right-4 z-40 w-80 rounded-lg border border-slate-200 bg-white shadow-lg">
      ...
      <div className="max-h-80 overflow-y-auto">
        {uploads.map((u) => {
          ...
          return (
            <div key={u.key} className="border-b border-slate-50 px-3 py-2 last:border-0">
              ...
              {u.status === 'completing' && <p className="text-[11px] text-slate-400">Finishing up…</p>}
              {u.status === 'incomplete' && (
